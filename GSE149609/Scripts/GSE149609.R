...
GSE149609 <- import("Data/GSE149609_raw_counts.tsv")
metadata <- import("Data/GSE149609_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE149609 |> column_to_rownames("GeneID") |> as.matrix()
...
coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("adjacent normal esophageal tissue", "normal", x))() |>
  (\(x) gsub("esophageal squamouse carcinoma", "cancerous", x))() |>
  as.factor()
