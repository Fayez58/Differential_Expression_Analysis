...
GSE29968 <- import("Data/GSE29968_raw_counts.tsv")
metadata <- import("Data/GSE29968_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE29968 |> column_to_rownames("GeneID") |> as.matrix()

coldata <- metadata[,c("Sample Name","tissue_type")] |> 
  column_to_rownames("Sample Name") |> 
  dplyr::rename("condition" = "tissue_type")

coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("non-tumor", "normal", x))() |>
  (\(x) gsub("tumor", "cancerous", x))() |>
  as.factor()
