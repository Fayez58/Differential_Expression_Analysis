...
GSE213565 <- import("Data/GSE213565_raw_counts.tsv")
metadata <- import("Data/GSE213565_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE213565 |> column_to_rownames("GeneID") |> as.matrix()

coldata <- metadata[,c("Sample Name","cell_type")] |> 
  column_to_rownames("Sample Name") |> 
  dplyr::rename("condition" = "cell_type")

coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("adjacent normal", "normal", x))() |>
