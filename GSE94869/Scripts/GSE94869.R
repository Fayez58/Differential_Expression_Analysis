...
GSE94869 <- import("Data/GSE94869_raw_counts.tsv")
metadata <- import("Data/GSE94869_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE94869 |> column_to_rownames("GeneID") |> as.matrix()
...
coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("matched non-malignant esophageal epithelial tissue", "normal", x))() |>
  (\(x) gsub("esophageal adenocarcinoma", "cancerous", x))() |>
  as.factor()
coldata <- coldata[coldata$condition != "cancerous cell line", , drop = FALSE]
