...
GSE234304 <- import("Data/GSE234304_raw_counts.tsv")
metadata <- import("Data/GSE234304_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE234304 |> column_to_rownames("GeneID") |> as.matrix()

coldata <- metadata[,c("Sample Name","tissue")] |> 
  column_to_rownames("Sample Name") |> 
  dplyr::rename("condition" = "tissue")

# Match coldata with count matrix
coldata <- coldata[coldata$condition != "Barretts esophagus", , drop = FALSE]
...
coldata <- coldata |>
  mutate(condition = ifelse(condition == "Non-tumor", "Nontumor", condition))
coldata$condition <- coldata$condition |> as.factor()
...
dds$condition <- relevel(dds$condition, ref="Nontumor")
dds <- DESeq(dds)
