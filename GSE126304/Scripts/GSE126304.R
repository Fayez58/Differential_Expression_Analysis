...
GSE126304 <- import("Data/GSE126304_raw_counts.tsv")
metadata <- import("Data/GSE126304_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE126304 |> column_to_rownames("GeneID") |> as.matrix()

coldata <- metadata[,c("Sample Name","tumor_or_normal")] |> 
  column_to_rownames("Sample Name") |> 
  dplyr::rename("condition" = "tumor_or_normal")

coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("paired normal", "normal", x))() |>
  (\(x) gsub("adenocarcinoma", "cancerous", x))() |>
  as.factor()
