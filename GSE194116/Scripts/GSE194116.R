...
GSE194116 <- import("Data/GSE194116_raw_counts.tsv")
metadata <- import("Data/GSE194116_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE194116 |> column_to_rownames("GeneID") |> as.matrix()

coldata <- metadata[,c("Sample Name","tissue_subtype")] |> 
  column_to_rownames("Sample Name") |> 
  dplyr::rename("condition" = "tissue_subtype")

coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("adjacent normal tissue", "normal", x))() |>
  (\(x) gsub("ESCC tumor", "cancerous", x))() |>
  as.factor()
...
dds <- dds[rowSums(counts(dds) >= 20) >= 6, ]
