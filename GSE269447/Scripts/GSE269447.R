...
GSE269447 <- import("Data/GSE269447_raw_counts.tsv")
metadata <- import("Data/GSE269447_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE269447 |> column_to_rownames("GeneID") |> as.matrix()
...
coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("normal esophageal.*", "normal", x))() |>
