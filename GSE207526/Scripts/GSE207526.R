...
GSE207526 <- import("Data/GSE207526_raw_counts.tsv")
metadata <- import("Data/GSE207526_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE207526 |> column_to_rownames("GeneID") |> as.matrix()
...
coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("healthy squamous cell tissue from patient with.*", "normal", x))() |>
