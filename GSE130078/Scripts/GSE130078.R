...
GSE130078 <- import("Data/GSE130078_raw_counts.tsv")
metadata <- import("Data/GSE130078_metadata.csv")

#formatting count data and metadata for differential expression analysis
count_data <- GSE130078 |> column_to_rownames("GeneID") |> as.matrix()
...
coldata$condition <- coldata$condition |>
  as.character() |>
  (\(x) gsub("ESCC adjacent normal tissue", "normal", x))() |>
  (\(x) gsub("ESCC tumor tissue", "cancerous", x))() |>
  as.factor()
